...
private:
    void UpdateUnits();
    void UpdateCities();
    void HandleSettler(std::shared_ptr<AUnit> Settler);
    void HandleCombatUnit(std::shared_ptr<AUnit> CombatUnit);

    FVector2 FindNearbyGoodTile(const FVector2& currentPos);
    std::shared_ptr<AUnit> FindNearestEnemy(const FVector2& currentPos);
